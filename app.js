...
const LAST_USERNAME_KEY = "libro-vivo-studio-last-username";
...
class LibroVivoStudio {
  constructor() {
    ...
    this.boundStickerPointerMove = this.handleStickerPointerMove.bind(this);
    this.boundStickerPointerUp = this.handleStickerPointerUp.bind(this);

    this.state = this.createDefaultState();

    this.elements = {
      addImageFrameButton: document.getElementById("addImageFrameButton"),
      addStickerButton: document.getElementById("addStickerButton"),
      addSpreadButton: document.getElementById("addSpreadButton"),
      authCreateButton: document.getElementById("authCreateButton"),
      authForm: document.getElementById("authForm"),
      authGate: document.getElementById("authGate"),
      authLoadButton: document.getElementById("authLoadButton"),
      ...
      bringStickerForwardButton: document.getElementById("bringStickerForwardButton"),
      ...
      coverStickerLayer: document.getElementById("coverStickerLayer"),
      ...
      imageStickerLayer: document.getElementById("imageStickerLayer"),
      ...
      removeStickerButton: document.getElementById("removeStickerButton"),
      ...
      textStickerLayer: document.getElementById("textStickerLayer"),
  ...
  bindEvents() {
    this.elements.authForm.addEventListener("submit", async (event) => {
      ...
      await this.submitAuthForm("load");
    });

    this.elements.authCreateButton.addEventListener("click", async () => {
      await this.submitAuthForm("create");
    });

    this.elements.authLoadButton.addEventListener("click", async () => {
      await this.submitAuthForm("load");
    });

    this.elements.switchUserButton.addEventListener("click", async () => {
    ...
    this.elements.addStickerButton.addEventListener("click", () => {
      this.elements.stickerInput.click();
    });

    this.elements.stickerInput.addEventListener("change", async (event) => {
      const files = Array.from(event.target.files || []);
      if (files.length) {
        await this.attachStickersToCurrentPage(files);
      }

      event.target.value = "";
    });

    this.elements.removeStickerButton.addEventListener("click", () => {
      this.removeSelectedSticker();
    });

    this.elements.bringStickerForwardButton.addEventListener("click", () => {
      this.bringSelectedStickerForward();
    });
    ...
    [this.elements.coverStickerLayer, this.elements.textStickerLayer, this.elements.imageStickerLayer]
      .forEach((layer) => {
        layer.addEventListener("click", (event) => {
          const sticker = event.target.closest(".page-sticker");
          this.selectSticker(sticker?.dataset.stickerId || null);
        });

        layer.addEventListener("pointerdown", (event) => {
          this.handleStickerPointerDown(event);
        });
      });
  ...
  async initializeCloud() {
    ...
    this.elements.authStateText.textContent = "Sin iniciar";
    this.elements.cloudStateText.textContent = this.getCloudStateLabel();
    this.elements.recoveryText.textContent = "Usa tu usuario y clave para crear un libro nuevo o cargar el que ya guardaste.";
    this.setSaveStatus("local", "Esperando inicio de sesion");
    this.showAuthGate();
  }

  async submitAuthForm(action = "load") {
    if (!this.firebaseEnabled) {
      return;
    }

    const username = this.elements.authUsernameInput.value;
    const password = this.elements.authPasswordInput.value;
    const normalizedUsername = normalizeUsername(username);

    if (!normalizedUsername) {
      this.setAuthMessage("Escribe un usuario valido.");
      return;
    }

    if (String(password || "").trim().length < 6) {
      this.setAuthMessage("La clave debe tener al menos 6 caracteres.");
      return;
    }

    this.setAuthBusy(true, action);
    this.setAuthMessage(action === "create" ? "Creando libro..." : "Cargando libro...");

    try {
      const session = action === "create"
        ? await createBookUser({ username: normalizedUsername, password })
        : await loadBookUser({ username: normalizedUsername, password });
      this.user = session.user;
      this.currentUsername = session.username;
      localStorage.setItem(LAST_USERNAME_KEY, this.currentUsername);
      this.elements.authPasswordInput.value = "";
      await this.loadBookForCurrentUser(action === "create");
      this.hideAuthGate();
      this.showToast(action === "create" ? "Libro creado correctamente." : "Libro cargado correctamente.");
    } catch (error) {
      console.error(error);
      this.setAuthMessage(this.getAuthErrorMessage(error, action));
    } finally {
      this.setAuthBusy(false, action);
    }
  }

  async loadBookForCurrentUser(wasCreated = false) {
    try {
      this.elements.authStateText.textContent = this.currentUsername;
      this.elements.cloudStateText.textContent = this.getCloudStateLabel();

      const remoteState = await fetchRemoteBook(this.user.uid);
      const localBackupEntry = this.getAvailableLocalBackup(this.currentUsername);
      const localBackup = localBackupEntry?.backup || null;
      ...
      if (selectedState) {
        ...
        this.elements.recoveryText.textContent = selectedState.source === "local"
          ? (localBackupEntry?.source === "legacy"
            ? "Se recupero una copia local anterior y se movera a este usuario."
            : "Se recupero una copia local de este usuario y se volvera a sincronizar.")
        ...
        this.elements.recoveryText.textContent = wasCreated
          ? "Tu libro nuevo ya esta listo para empezar."
          : "No habia un libro previo guardado para este usuario.";
      }
      ...
      if (permissionDenied && this.user) {
        this.elements.authStateText.textContent = this.currentUsername || "Sin iniciar";
        this.elements.cloudStateText.textContent = "Revisa reglas";
        this.elements.recoveryText.textContent = "Firebase deja entrar al usuario, pero Firestore esta bloqueando el libro por reglas.";
        this.setSaveStatus("local", "Guardado local hasta corregir Firestore.");
        this.setAuthMessage("Debes actualizar las reglas de Firestore para usuarios con login.");
        return;
      }

      this.elements.authStateText.textContent = "Error";
      this.elements.cloudStateText.textContent = "Solo respaldo local";
      this.elements.recoveryText.textContent = "No se pudo recuperar el libro de este usuario. El respaldo local sigue protegido.";
      this.setSaveStatus("local", "Sesion nube no disponible.");
      this.setAuthMessage("No se pudo abrir este usuario en la nube.");
    }
  }
  ...
  prefillUsername() {
    ...
    this.elements.authUsernameInput.value = normalizeUsername(savedUsername);
  }

  showAuthGate() {
    this.authGateOpen = true;
    document.body.classList.add("auth-locked");
    this.elements.authGate.classList.add("show");
    this.elements.authPasswordInput.value = "";
    this.elements.authMessageText.textContent = "";
    this.elements.authUsernameInput.focus();
  }

  hideAuthGate() {
    this.authGateOpen = false;
    document.body.classList.remove("auth-locked");
    this.elements.authGate.classList.remove("show");
  }
  ...
  setAuthBusy(isBusy, action = "load") {
    this.elements.authCreateButton.disabled = isBusy;
    this.elements.authLoadButton.disabled = isBusy;
    this.elements.authUsernameInput.disabled = isBusy;
    this.elements.authPasswordInput.disabled = isBusy;
    this.elements.authCreateButton.textContent = isBusy && action === "create" ? "Creando..." : "Crear libro";
    this.elements.authLoadButton.textContent = isBusy && action === "load" ? "Cargando..." : "Cargar libro";
  }

  getAuthErrorMessage(error, action = "load") {
    const code = String(error?.code || "");

    if (code === "auth/email-already-in-use") {
      return "Ese usuario ya existe. Usa Cargar libro.";
    }

    if (code === "auth/user-not-found" || (code === "auth/invalid-login-credentials" && action === "load")) {
      return "Ese usuario no existe. Usa Crear libro.";
    }

    if (code === "auth/invalid-login-credentials" || code === "auth/wrong-password") {
      return "La clave no coincide con ese usuario.";
    }

    if (code === "auth/too-many-requests") {
      return "Hubo muchos intentos. Espera un momento y vuelve a probar.";
    }

    if (code === "auth/user-disabled") {
      return "Este usuario fue deshabilitado en Firebase.";
    }

    if (code === "auth/operation-not-allowed") {
      return "Solo falta activar Email/Password en Firebase. Luego podras entrar con usuario simple como katerin.";
    }

    if (code === "auth/network-request-failed") {
      return "No hay conexion para iniciar sesion.";
    }
  ...
  async changeUser() {
    ...
    this.elements.authStateText.textContent = "Sin iniciar";
    this.elements.cloudStateText.textContent = this.getCloudStateLabel();
    this.elements.recoveryText.textContent = "Escribe otro usuario para crear o cargar otro libro.";
    this.renderAll();
    this.showAuthGate();
  }
    ...
    return {
      state: {
        book: {
          ...
          coverStickers: this.normalizeStickerItems(incomingBook.coverStickers, 0, 18),
  ...
  normalizeStickerItems(items, baseOffset = 0, defaultSize = 18) {
    return Array.isArray(items)
      ? items.map((item, index) => ({
          id: item.id || this.makeId(),
          src: item.src || "",
          x: this.clamp(Number(item.x ?? 8 + baseOffset + index * 3), 0, 88),
          y: this.clamp(Number(item.y ?? 10 + index * 3), 0, 88),
          w: this.clamp(Number(item.w ?? defaultSize), 8, 52),
          h: this.clamp(Number(item.h ?? defaultSize), 8, 52),
  ...
  normalizeSpread(spread, index) {
    ...
    return {
      ...
      imageItems,
      textStickers: this.normalizeStickerItems(spread.textStickers, 0, 16),
      imageStickers: this.normalizeStickerItems(spread.imageStickers, 2, 18),
  ...
  renderCurrentPage() {
    ...
    const showEditingChrome = !this.presentationMode;
    ...
    this.elements.stickerTools.classList.toggle("hidden", this.presentationMode);
  ...
  downloadAsPdf() {
    ...
    this.showToast("Libro listo: elige Guardar como PDF en la ventana de impresion.");
  }

  buildPrintStickerMarkup(stickers = []) {
    return (stickers || []).map((sticker) => `
      <div class="print-sticker" style="left:${sticker.x}%;top:${sticker.y}%;width:${sticker.w}%;height:${sticker.h}%;z-index:${sticker.z};">
        ${sticker.src ? `<img src="${this.escapeAttribute(sticker.src)}" alt="Sticker del libro">` : ""}
      </div>
    `).join("");
  }
  ...
  buildPrintDocument() {
    const coverImage = this.state.book.coverImage?.src
      ? `background-image: linear-gradient(180deg, rgba(5, 8, 15, 0.12), rgba(5, 8, 15, 0.84)), url('${this.escapeAttribute(this.state.book.coverImage.src)}');`
      : "background-image: linear-gradient(160deg, #03070e 8%, #0b1830 48%, #47bff7 78%, #ffd84d 100%);";

    const sheets = [
      `
        <section class="print-sheet print-cover" style="${coverImage}">
          <div class="print-cover-shell">
            <div class="print-sticker-board">
              ${this.buildPrintStickerMarkup(this.state.book.coverStickers)}
            </div>
        ...
        <section class="print-sheet">
          <article class="print-page print-text" style="--sheet-color:${textColor}">
            ...
            <div class="print-sheet-body">
              ...
              <div class="print-sticker-board">
                ${this.buildPrintStickerMarkup(spread.textStickers)}
              </div>
            </div>
          </article>
        </section>
      ...
      sheets.push(`
        <section class="print-sheet">
          <article class="print-page print-image" style="--sheet-color:${imageColor}">
            <div class="print-page-number">${pageNumberImage}</div>
            <header class="print-sheet-head">
              <span>Imagen</span>
              <strong>${title}</strong>
            </header>
            <div class="print-sheet-body">
              ...
              <div class="print-sticker-board">
                ${this.buildPrintStickerMarkup(spread.imageStickers)}
              </div>
            </div>
          </article>
        </section>
...
<body>
  ...
  getStickerLayerForDescriptor(descriptor = this.getCurrentPageDescriptor()) {
    if (descriptor.type === "cover") {
      return this.elements.coverStickerLayer;
    }

    if (descriptor.type === "text") {
      return this.elements.textStickerLayer;
    }

    if (descriptor.type === "image") {
      return this.elements.imageStickerLayer;
    }

    return null;
  }

  getCurrentStickerCollection(descriptor = this.getCurrentPageDescriptor()) {
    if (descriptor.type === "cover") {
      this.state.book.coverStickers = this.state.book.coverStickers || [];
      return this.state.book.coverStickers;
    }
    ...
    if (descriptor.type === "text") {
      descriptor.spread.textStickers = descriptor.spread.textStickers || [];
      return descriptor.spread.textStickers;
    }

    if (descriptor.type === "image") {
      descriptor.spread.imageStickers = descriptor.spread.imageStickers || [];
      return descriptor.spread.imageStickers;
    }
  ...
  renderStickerLayers() {
    [
      this.elements.coverStickerLayer,
      this.elements.textStickerLayer,
      this.elements.imageStickerLayer,
    ].forEach((layer) => {
      layer.innerHTML = "";
    });

    const descriptor = this.getCurrentPageDescriptor();
    const layer = this.getStickerLayerForDescriptor(descriptor);
    const stickers = [...this.getCurrentStickerCollection(descriptor)].sort((left, right) => left.z - right.z);
    ...
    for (const sticker of stickers) {
      ...
      node.className = `page-sticker${sticker.id === this.selectedStickerId ? " selected" : ""}`;
      node.dataset.stickerId = sticker.id;
      this.applyStickerStyle(node, sticker);
      node.innerHTML = `
        <img src="${this.escapeAttribute(sticker.src)}" alt="Sticker del libro">
        ${this.presentationMode ? "" : '<div class="sticker-toolbar">Sticker</div><div class="sticker-resize-handle" aria-hidden="true"></div>'}
      `;
      layer.appendChild(node);
    }
  }

  applyStickerStyle(node, sticker) {
    node.style.left = `${sticker.x}%`;
    node.style.top = `${sticker.y}%`;
    node.style.width = `${sticker.w}%`;
    node.style.height = `${sticker.h}%`;
    node.style.zIndex = String(sticker.z);
  }
  ...
  createStickerItem(index = 0) {
    return {
      id: this.makeId(),
      src: "",
      x: this.clamp(8 + index * 4, 4, 84),
      y: this.clamp(10 + index * 3, 4, 84),
      w: 18,
      h: 18,
      z: index + 1,
    };
  ...
  selectSticker(stickerId) {
    ...
    const layer = this.getStickerLayerForDescriptor();
    if (!layer) {
      return;
    }

    for (const sticker of layer.querySelectorAll(".page-sticker")) {
      sticker.classList.toggle("selected", sticker.dataset.stickerId === stickerId);
    }
  }

  getSelectedSticker() {
    return this.getCurrentStickerCollection().find((sticker) => sticker.id === this.selectedStickerId) || null;
  }

  getStickerOwnerDescriptor() {
    const descriptor = this.getCurrentPageDescriptor();
    return {
      type: descriptor.type,
      spread: descriptor.spread || null,
    };
  }

  touchStickerOwner(owner = this.getStickerOwnerDescriptor()) {
    if (owner.type === "cover") {
      this.touchBook();
      return;
    }
    ...
    owner.spread.updatedAtClient = Date.now();
    this.touchBook();
    this.markSpreadDirty(owner.spread.id);
  }

  removeSelectedSticker() {
    const collection = this.getCurrentStickerCollection();
    if (!this.selectedStickerId || !collection.length) {
      this.showToast("Selecciona un sticker primero.");
      return;
    }

    const owner = this.getStickerOwnerDescriptor();
    const nextItems = collection.filter((item) => item.id !== this.selectedStickerId);

    if (owner.type === "cover") {
      this.state.book.coverStickers = nextItems;
    } else if (owner.type === "text") {
      owner.spread.textStickers = nextItems;
    } else if (owner.type === "image") {
      owner.spread.imageStickers = nextItems;
    }

    this.pendingUploads = this.pendingUploads.filter((upload) => upload.stickerId !== this.selectedStickerId);
    this.selectedStickerId = null;
    this.touchStickerOwner(owner);
    this.renderStickerLayers();
    this.persistLocalBackup();
    this.queueSave();
  }

  bringSelectedStickerForward() {
    const sticker = this.getSelectedSticker();
    const collection = this.getCurrentStickerCollection();

    if (!sticker || !collection.length) {
      this.showToast("Selecciona un sticker primero.");
      return;
    }

    sticker.z = Math.max(...collection.map((entry) => entry.z), 0) + 1;
    this.touchStickerOwner();
    this.renderStickerLayers();
    this.persistLocalBackup();
    this.queueSave();
  }

  handleStickerPointerDown(event) {
    if (this.presentationMode) {
      return;
    }

    const stickerNode = event.target.closest(".page-sticker");
    if (!stickerNode) {
      return;
    }

    const sticker = this.getCurrentStickerCollection().find((entry) => entry.id === stickerNode.dataset.stickerId);
    const layer = this.getStickerLayerForDescriptor();
    const descriptor = this.getCurrentPageDescriptor();
    ...
    const mode = event.target.closest(".sticker-resize-handle") ? "resize" : "drag";
    this.selectSticker(sticker.id);
    this.stickerInteraction = {
      mode,
      pageType: descriptor.type,
      spreadId: descriptor.spread?.id || null,
    ...
    window.addEventListener("pointermove", this.boundStickerPointerMove);
    window.addEventListener("pointerup", this.boundStickerPointerUp);
    event.preventDefault();
  }

  handleStickerPointerMove(event) {
    if (!this.stickerInteraction) {
      return;
    }

    const descriptor = this.getCurrentPageDescriptor();
    if (descriptor.type !== this.stickerInteraction.pageType) {
      return;
    }

    if ((descriptor.spread?.id || null) !== this.stickerInteraction.spreadId) {
      return;
    }

    const sticker = this.getCurrentStickerCollection(descriptor)
      .find((entry) => entry.id === this.stickerInteraction.stickerId);

    if (!sticker) {
      return;
    }

    const dx = ((event.clientX - this.stickerInteraction.startClientX) / this.stickerInteraction.rect.width) * 100;
    const dy = ((event.clientY - this.stickerInteraction.startClientY) / this.stickerInteraction.rect.height) * 100;

    if (this.stickerInteraction.mode === "drag") {
      sticker.x = this.clamp(this.stickerInteraction.startX + dx, 0, 100 - sticker.w);
      sticker.y = this.clamp(this.stickerInteraction.startY + dy, 0, 100 - sticker.h);
    } else {
      sticker.w = this.clamp(this.stickerInteraction.startW + dx, 8, 100 - sticker.x);
      sticker.h = this.clamp(this.stickerInteraction.startH + dy, 8, 100 - sticker.y);
    }

    const stickerNode = this.getStickerLayerForDescriptor(descriptor)
      ?.querySelector(`.page-sticker[data-sticker-id="${sticker.id}"]`);
    if (stickerNode) {
      this.applyStickerStyle(stickerNode, sticker);
    }
  }
  ...
  handleStickerPointerUp() {
    ...
    const descriptor = this.getCurrentPageDescriptor();
    if (descriptor.type === this.stickerInteraction.pageType
      && (descriptor.spread?.id || null) === this.stickerInteraction.spreadId) {
      this.touchStickerOwner({
        type: descriptor.type,
        spread: descriptor.spread || null,
      });
    ...
    this.stickerInteraction = null;
    window.removeEventListener("pointermove", this.boundStickerPointerMove);
    window.removeEventListener("pointerup", this.boundStickerPointerUp);
  }
  ...
  async attachStickersToCurrentPage(files) {
    const descriptor = this.getCurrentPageDescriptor();
    if (!(descriptor.type === "cover" || descriptor.type === "text" || descriptor.type === "image")) {
      this.showToast("Abre una pagina del libro para agregar stickers.");
      return;
    }

    const collection = this.getCurrentStickerCollection(descriptor);

    for (const file of files.filter((entry) => entry?.type?.startsWith("image/"))) {
      const dataUrl = await this.fileToDataUrl(file);
      const sticker = this.createStickerItem(collection.length);
      sticker.src = dataUrl;
      sticker.z = Math.max(...collection.map((entry) => entry.z), 0) + 1;
      collection.push(sticker);
      this.selectedStickerId = sticker.id;

      this.pendingUploads = this.pendingUploads.filter((upload) => upload.stickerId !== sticker.id);
      this.pendingUploads.push({
        id: this.makeId(),
        kind: "sticker",
        pageType: descriptor.type,
        spreadId: descriptor.spread?.id || null,
        stickerId: sticker.id,
        fileName: file.name || `sticker-${Date.now()}.png`,
        mimeType: file.type || "image/png",
        dataUrl,
        createdAtClient: Date.now(),
      });
    ...
    this.persistLocalBackup();
    this.updateBackupStatus();
    this.showToast("Sticker agregado. Puedes moverlo y cambiar su tamano.");
  ...
  findStickerCollectionByUpload(upload) {
    if (upload.pageType === "cover") {
      this.state.book.coverStickers = this.state.book.coverStickers || [];
      return {
        collection: this.state.book.coverStickers,
        spread: null,
      };
    }
    ...
    if (upload.pageType === "text") {
      spread.textStickers = spread.textStickers || [];
      return {
        collection: spread.textStickers,
        spread,
      };
    }

    spread.imageStickers = spread.imageStickers || [];
    return {
      collection: spread.imageStickers,
      spread,
    };
  }
  ...
  async processPendingUploads() {
    ...
    if (!this.user) {
      this.updateBackupStatus();
      this.setSaveStatus("local", "Inicia sesion para sincronizar imagenes.");
      return;
    }
    ...
    for (const upload of this.pendingUploads) {
      ...
      if (upload.kind === "sticker") {
        const { collection, spread } = this.findStickerCollectionByUpload(upload);
        const sticker = collection?.find((item) => item.id === upload.stickerId);

        if (!collection || !sticker) {
          continue;
        }

        try {
          this.setSaveStatus("saving", "Subiendo sticker...");
          const file = this.dataUrlToFile(upload.dataUrl, upload.fileName, upload.mimeType);
          const result = await uploadImageToCloudinary({
            file,
            uid: this.user.uid,
            spreadId: upload.pageType === "cover"
              ? "portada-stickers"
              : `${upload.spreadId}-${upload.pageType}-stickers`,
            imageId: upload.stickerId,
          });
          sticker.src = result.secureUrl;
          if (spread) {
            spread.updatedAtClient = Date.now();
            this.markSpreadDirty(spread.id);
          }
          ...
          console.error("No se pudo subir el sticker.", error);
          remaining.push(upload);
        }
  ...
  async saveNow(options = {}) {
    ...
    if (!this.firebaseEnabled || !this.user) {
      this.setSaveStatus("local", this.firebaseEnabled ? "Inicia sesion para sincronizar." : "Solo respaldo local por ahora.");
      return;
    }
  ...
  getBackupStorageKey(username = this.currentUsername) {
    const normalizedUsername = normalizeUsername(username);
    return normalizedUsername ? `${LOCAL_BACKUP_KEY}:${normalizedUsername}` : LOCAL_BACKUP_KEY;
  }

  getLocalBackup(username = this.currentUsername) {
    try {
      const raw = localStorage.getItem(this.getBackupStorageKey(username));
  ...
  getLegacyBackup() {
    try {
      ...
      console.error("No se pudo leer el respaldo local antiguo.", error);
      return null;
    }
  }

  getAvailableLocalBackup(username = this.currentUsername) {
    const userBackup = this.getLocalBackup(username);
    ...
    const legacyBackup = this.getLegacyBackup();
    const normalizedUsername = normalizeUsername(username);
    const legacyUsername = normalizeUsername(legacyBackup?.username || "");

    if (!legacyBackup) {
      return null;
    }

    if (legacyUsername && normalizedUsername && legacyUsername !== normalizedUsername) {
      return null;
    }
  ...
  persistLocalBackup() {
    const snapshot = {
      ...
      username: this.currentUsername || null,
    };

    try {
      localStorage.setItem(this.getBackupStorageKey(), JSON.stringify(snapshot));
    } catch (error) {
