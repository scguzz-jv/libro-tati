...
function usernameToEmail(username) {
  ...
  return `u${encoded}@librotati.app`;
}
...
function validateCredentials(username, password) {
  ...
  if (!normalizedUsername) {
    throw new Error("Escribe un usuario valido.");
  }
...
export async function createBookUser({ username, password }) {
  ...
  const { normalizedUsername, normalizedPassword } = validateCredentials(username, password);
  const email = usernameToEmail(normalizedUsername);
  const credential = await createUserWithEmailAndPassword(authInstance, email, normalizedPassword);
...
export async function loadBookUser({ username, password }) {
  const init = await initializeFirebaseServices();
  ...
  const { normalizedUsername, normalizedPassword } = validateCredentials(username, password);
  const email = usernameToEmail(normalizedUsername);
  const credential = await signInWithEmailAndPassword(authInstance, email, normalizedPassword);
...
export async function loginWithUsername({ username, password }) {
  return loadBookUser({ username, password });
}
...
export async function saveRemoteBook(uid, payload) {
  ...
  batch.set(
    bookRef,
    {
      ...
      coverStickers: Array.isArray(payload.book.coverStickers) ? payload.book.coverStickers : [],
  ...
  for (const spread of payload.spreads) {
    ...
    batch.set(
      spreadRef,
      {
        ...
        textStickers: Array.isArray(spread.textStickers) ? spread.textStickers : [],
        imageStickers: Array.isArray(spread.imageStickers) ? spread.imageStickers : [],
